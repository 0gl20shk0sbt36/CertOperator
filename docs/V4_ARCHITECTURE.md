...
|------|------|
| `priority` 不同 | 数字小的优先 |
| `priority` 相同 | **配置顺序靠后的覆盖靠前的**（最后写入的为最终状态） |
| 时间窗口交叉 | 自动切割为不冲突的子时间段 |
| `clients` 精确匹配 | 在服务端匹配客户端 CN |

`priority` 相同时的覆盖规则：

```
规则 A: priority=10, clients=null,             09-12: mode=passwordless
规则 B: priority=10, clients=["laptop-alice"],  09-12: mode=deny

两者 priority 相同。
规则 B 配置顺序靠后，覆盖规则 A 中对 laptop-alice 的部分。
结果：
  09-12: 默认 passwordless（规则A）
  09-12: laptop-alice → deny（规则B 覆盖）
```

### 执行层（自动编译）

规则配置层输入，按优先级编译。同优先级按配置顺序靠后覆盖靠前。遇到冲突的时间窗口自动拆分为不冲突的多个时间段。每个时间段对应唯一的 issue 和 judge 状态。
