...
```bash
cert-operator init                        # 首次初始化（生成 SSH CA + HTTPS 证书 + mTLS CA + admin 证书）
...
cert-operator clients <action> [args...]  # mTLS 客户端证书管理（issue/list/show/revoke）
...
init 自动完成：
  ...
  4. 生成 mTLS CA 证书（用于签发独立客户端证书）
  5. 签发默认 admin 客户端证书 → data/clients/admin.tar.gz
  6. 输出目标服务器配置指引
```
...
## clients — mTLS 客户端证书管理

管理独立签发的 mTLS 客户端证书。每个客户端使用独立证书，服务端维护 `data/clients.json` 名单，
TLS 握手时验证证书序列号是否在名单中（撤销即从名单删除，立即生效）。

### 签发客户端证书

```bash
# 基本用法
cert-operator clients issue laptop-alice "alice@example.com"

# 指定有效期、SAN、关联 SSH 用户
cert-operator clients issue prod-server "devops team" \
  --validity 90 \
  --san "IP:10.0.0.5,DNS:prod.internal" \
  --user deploy
```
...
| 参数 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `name` | ✅ | — | 证书标识名，如 `laptop-alice` |
| `granted-to` | ✅ | — | 授予者，如 `alice@example.com` |
| `--validity` | | 365 | 有效期（天） |
| `--san` | | 无 | Subject Alternative Name，如 `IP:10.0.0.5,DNS:host` |
| `--user` | | 无 | 关联的 SSH 用户名 |

#### 输出

```
✅ Client cert issued:  laptop-alice
📦 Package:            /opt/ca_server/data/clients/laptop-alice.tar.gz
🔑 Serial:             3
📅 Expires:            2027-06-11 15:00:00 UTC
👤 Granted to:         alice@example.com
```
...
```bash
cert-operator clients show laptop-alice
```

### 撤销（立即生效）

```bash
cert-operator clients revoke laptop-alice
```

撤销后 TLS 握手时拒绝该证书，无需重启服务。

### 部署到客户端

```bash
# 从服务器复制证书包
scp root@ca-server:/opt/ca_server/data/clients/laptop-alice.tar.gz ./

# 用 CLI 解压部署
cert-operator deploy-client laptop-alice.tar.gz

# 或手动解压
tar -xzf laptop-alice.tar.gz -C ~/.hermes/certs/
```
...
├── laptop-alice.key     # 客户端私钥（权限 600）
└── laptop-alice.cert    # 客户端证书（mTLS CA 签发）
```
